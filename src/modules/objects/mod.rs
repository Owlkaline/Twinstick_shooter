...
use maat_graphics::cgmath::{Vector2, Vector3, Vector4, Zero, Euler, Quaternion};
...
pub trait GenericObject {
  ...
  fn update_collision_data(&mut self, model_data: &Vec<ModelData>, override_size: Option<Vector3<f32>>) {
    ...
    for i in 0..model_data.len() {
      if model_data[i].name() == self.data().model.to_string() {
        ...
        if override_size.is_some() {
          bounding_box_size = override_size.unwrap();
        } else {
        ...
        for j in 0..model_data[i].num_collision_info() {
          ...
          let mut size = model_data[i].collision_info()[j].size();
     ...
     for i in 0..self.data().collision_data.len() {
       match self.data().collision_data[i] {
         CollisionType::AABB(a_pos, a_size, quaternion) => {
          ...
          // let mut rotation = Vector3::new(math::to_degrees(rot.x.0), math::to_degrees(rot.z.0), math::to_degrees(rot.y.0));
           let rotation = Vector3::new(0.0, 90.0, 90.0);
